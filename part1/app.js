    ...
    try {
      ...
      `);
        ...
        if (request_count[0].count === 0) {
            ...
            ((SELECT dog_id FROM Dogs WHERE name = 'Toutou'), '2025-06-12 10:00:00', 60, 'China', 'open'),
            ((SELECT dog_id FROM Dogs WHERE name = 'Guodong'), '2025-06-12 13:00:00', 60, 'Qingdao', 'open');
                `);
        }
...
app.get('/api/walkrequests/open',async (req, res) => {
    try{
        const [requests] = await db.query("SELECT wr.request_id, d.name AS dog_name, wr.requested_time, wr.duration_minutes, wr.location, u.username AS owner_username FROM WalkRequest wr JOIN Dogs d ON wr.dog_id = d.dog_id JOIN Users u ON d.owner_id = u.user_id WHERE wr.status ='open'");
        res.json(dogs);
    }
    catch(err){
        console.log(err);
